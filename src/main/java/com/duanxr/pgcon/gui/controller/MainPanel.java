...
public class MainPanel {
  ...
  private void initializeScreenAndCanvas() {
    ...
    displayHandler.registerScreen(
        input -> Platform.runLater(() -> SwingFXUtils.toFXImage(input, screenImage)));
  ...
  private void initializeScriptConfigurations() {
    scriptManager.getMainScripts().values().stream()
        .filter(script -> script instanceof ConfigurableMainScript)
        .map(script -> (ConfigurableMainScript) script).forEach(this::bindScriptConfigurationToGUI);
  ...
  private void bindScriptConfigurationToGUI(ConfigurableMainScript configurableMainScript) {
    String scriptName = configurableMainScript.getScriptName();
    String cacheKey = getScriptConfigurationCacheKey(scriptName);
    Object registerConfig = configurableMainScript.registerConfig();
    try {
      CacheUtil.loadCache(cacheKey, registerConfig);
    } catch (Exception e) {
      guiLogger.warn("load script {} config cache failed", scriptName,
          e);
    }
    FXForm<?> configForm = new FXForm<>(registerConfig);
    setScriptConfigurationFormStyle(configForm);
    dynamicConfigurationMap.put(scriptName, configForm);
  }
  ...
  private void loadScript(String scriptName) {
    if (!Strings.isNullOrEmpty(scriptName)) {
      ...
      if (script instanceof ConfigurableMainScript) {
