...
# Convert imageInput source file list to differentiate them by type
...
ifneq ($(UNKNOWN_SOURCE),)
   $(error Unknown imageInput source file formats: $(UNKNOWN_SOURCE))
endif

# Convert imageInput source filenames into a list of required output object files
OBJECT_FILES += $(addsuffix .o, $(basename $(SRC)))

# Check if an output object file directory was specified instead of the imageInput file location
ifneq ($(OBJDIR),.)
...
# make, the imageInput source file doesn't exist and an error needs to be presented to the user
$(SRC):
	$(error Source file does not exist: $@)

# Compiles an imageInput C source file and generates an assembly listing for it
%.s: %.c $(MAKEFILE_LIST)
...
# Compiles an imageInput C source file and generates a linkable object file for it
...
# Assembles an imageInput ASM source file and generates a linkable object file for it
...
# Creates an assembly listing file from an imageInput project ELF file, containing interleaved assembly and source data
%.lss: %.elf
	@echo $(MSG_OBJDMP_CMD) Extracting LSS file data from \"$<\"
	$(CROSS)-objdump -h -d -S -z $< > $@

# Creates a symbol file listing the loadable and discarded symbols from an imageInput project ELF file
%.sym: %.elf
